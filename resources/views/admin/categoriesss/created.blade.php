...
@section('title', trans('New Category'))
@section('min-w', 'min-w-[40%]')
@section('max-h', 'max-h-[38vh] ')
@section('body')
    <form id="form" class="w-full" enctype="multipart/form-data" action="{{ route('createdCategory') }}" method="POST">
        @csrf
        ...
        <div class="w-full flex flex-col items-center space-y-4">
            ...
            <div class="form-group w-full ">
                <label>{{ __('Category name') }}</label>
                <input type="text" class="h-10 w-full  form-control text-black" id="name" name="name"
...
<script>
    $(document).ready(function() {
        ...
        var route = '{{ route('createdCategory') }}';
