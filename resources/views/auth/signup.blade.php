...
@section('content')
    <form enctype="multipart/form-data" action="{{ route('signup.store') }}" method="POST">
        @csrf
        <div class="flex flex-col  items-center space-y-6 ">
            ...
            <div class="space-x-2 input px-2">
                <input id="profile" aria-describedby="user_avatar_help" name="profile" type="file"
                    value="{{ old('profile') }}">
            ...
            <div class="space-x-2 input px-2">
                <input
                    id="name" type="text" name="name" placeholder="{{ __('Your Name') }}" autocomplete="name"
                    value="{{ old('name') }}">
            ...
            <div class="space-x-2 input px-2">
                <input
                    id="email" type="text" name="email" placeholder="{{ __('Email') }}" autocomplete="email"
                    value="{{ old('email') }}">
            ...
            <div class="w-full flex justify-center space-x-2  text-sm text-slate-100">
                <a 
                    href="{{ route('forgotPassword.index') }}">{{ __('Forgot Password') }}</a>
                <p>|</p>
            ...
            <button type="submit"
                class="submit bg-primaryColor-500 font-semibold text-[#0c2440]">{{ __('Sign up') }}</button>
        </div>
...
@section('content')
    <div class="w-screen h-screen flex justify-center items-center">
        <div class="w-full max-w-[50%] bg-white  p-5 border border-gray-100 shadow-xl rounded-xl">

            <img class="h-10 w-10" src="{{ asset('assets/auth/img/logo.png') }}" alt="">
            ...
            <form id="loginForm" action="{{ route('login.store') }}" method="POST">
                ...
                <div class="mt-4 relative">
                    ...
                    <button type="submit" id="showPassword"
                    ...
                    <button type="submit" id="hidePassword"
...
@endsection
@section('script')
    <script src="{{ asset('assets/general/js/login.js') }}"></script>
@endsection --}}
