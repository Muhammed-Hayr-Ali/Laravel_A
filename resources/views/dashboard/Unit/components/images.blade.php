@if (isset($unit->image) && $unit->image != null)
    <div class="col-12 border border-secondary-subtle rounded-2 p-1 text-center ">
        <a href="{{ asset($unit->image) }}" class="image-popup" data-lightbox="roadtrip">
            <img class="itemImage" src="{{ asset($unit->image) }}" alt="">
        </a>
        <div class="container">
            <div class="row align-items-center">
                <div class="col-auto m-0 p-0"><button class="delete btn btn-circle" data-id="{{ $unit->id }}"
                        data-name="{{ basename($unit->image) }}">
                        <i class="fa fa-trash" aria-hidden="true"></i>
                    </button>
                </div>
                <div class="col text-center">
                    <h7>{{ basename($unit->image) }}</h7>
                </div>
            </div>
        </div>
    </div>
