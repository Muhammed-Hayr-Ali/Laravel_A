        ...
        <div class="card-body">
                        ...
                        <div class="search-input">
                            ...
                            <div id="DataTables_Table_0_filter" class="dataTables_filter"><label>
                                    <input id="input" oninput="search()" type="search"
                                        class="form-control form-control-sm" placeholder="{{ __('brandList.Search...') }}"
                    ...
                    <form action="{{ route('filters') }}" method="POST">
                        ...
                        <div class="w-full flex flex-row space-x-2 pb-4">
                            ...
                            <div class="flex flex-row items-center">
                                <div class="pl-1">{{__('brandList.Category') }}</div>
                                <div class="w-40">
                                    <select class="select" name="category">
                                        <option value="all">{{__('brandList.All') }}</option>
                                        @php
                                            $categories = \App\Models\Category::all();
                                        @endphp
                                        @foreach ($categories as $brand)
                                            <option value="{{ $brand->id }}"
                                                @if (request()->input('category') == $brand->id) selected @endif>
                                                {{ __('category.' . $brand->name) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="flex flex-row items-center">
                                <div class="pl-1">{{__('brandList.Brand') }}</div>
                                <div class="w-40">
                            ...
                            <div class="flex flex-row items-center">
                                <div class="pl-1">{{__('brandList.Unit') }}</div>
                                <div class="w-40">
            ...
            @if (isset($brands) && $brands->count() > 0)
                                        ...
                                        <td class="">
                                            <div class="flex flex-row space-x-2 items-center h-full">
                                                ...
                                                <div class="product-img">
                                                    @if ($brand->image != null)
                                                        <a href="{{ asset($brand->image) }}"class="image-popup"
                                                            data-lightbox="image-1"> <img class="object-cover"
                                                                src="{{ asset($brand->image) }}" alt="category"></a>
                                                    @else
                ...
                <div class="flex flex-row justify-between pt-8">
                    <div class=""> {{ $brands->links('dashboard.pagination.default') }}</div>

                    <div class="flex  flex-row items-center">
                        <p>{{ __('brandList.Show per page') }}</p>
                        <form action="{{ route('Category.index') }}" method="GET">
                            <select name="perPage" onchange="this.form.submit()"
                                class=" mr-1 custom-select
                        custom-select-sm form-control form-control-sm">
                                <option {{ $brands->perPage() == 10 ? 'selected' : '' }}>10</option>
                                <option {{ $brands->perPage() == 25 ? 'selected' : '' }}>25</option>
                                <option {{ $brands->perPage() == 50 ? 'selected' : '' }}>50</option>
                                <option {{ $brands->perPage() == 100 ? 'selected' : '' }}>100</option>
                            </select>
                        </form>
                    </div>
                ...
                <div class="card h-72 ">
                    <div class="h-full flex flex-col justify-center items-center space-y-3 ">
                        <img style="width: 128px;" src="{{ asset('dashboard/assets/img/icons/empty-box.png') }}"
                            alt="img">
                        <h4>{{ __('brandList.No Data Found') }}</h4>
                    </div>
                </div>
            @endif
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            //         Swal.fire({
            //             title: "{{ __('brandList.Error') }}",
            //             text: message,
            //             icon: "error",
            //             confirmButtonText: "{{ __('brandList.Ok') }}",
            //         });
